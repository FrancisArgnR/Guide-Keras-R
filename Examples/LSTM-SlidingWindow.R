...
####################################################
### Reshape the data for LSTM (batch_size -> None, timesteps -> 1, features -> window width)

x_train <- array(x.train, dim = c(nrow(x.train), 1, ncol(x.train)))
y_train <- array(y.train, dim = c(nrow(y.train), 1, ncol(y.train)))
x_test <- array(x.test, dim = c(nrow(x.test), 1, ncol(x.test)))
y_test <- array(y.test, dim = c(nrow(y.test), 1, ncol(y.test)))
...
model %>% 
  layer_lstm(units = 8, activation = 'tanh', input_shape = c(1,ii), return_sequences = TRUE) %>% 
  #layer_lstm(units = 8, activation = 'tanh', batch_input_shape = dim(x_train),  return_sequences = TRUE) %>% 
...
# Fit the model
history <- model %>% fit(x_train, y_train, epochs = 100, batch_size=1, validation_data = list(x_test, y_test))
...
lines((k+1+ii):(length(kilos)),test,col="red")
